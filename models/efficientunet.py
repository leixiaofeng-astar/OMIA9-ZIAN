...
class EfficientUnet(nn.Module):
    def __init__(self, encoder, out_channels=2, concat_input=True):
        ...
        if self.concat_input:
            ...
            self.double_conv_input = double_conv(64, 32)
    ...
    def forward(self, x):
        ...
        x = x.to(device=cuda)
        x = torch.cat([x, blocks.popitem()[1]], dim=1)
        feature = self.double_conv4(x)

        if self.concat_input:
            feature = self.up_conv_input(feature)
            feature = torch.cat([feature, input_], dim=1)
            feature = self.double_conv_input(feature)
        else:
            feature = self.double_conv_input(feature)

        x = self.final_conv(feature)
